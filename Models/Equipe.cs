...
namespace E_JOGOS.Models
{
...

    {
        ...
        {
            List<Equipe> ListaEquipes = ReadAll();
            
                           
           ListaEquipes.RemoveAll(item => item.IdEquipe == idEquipe);

            
          StreamWriter file = new StreamWriter(path);
            
            foreach (Equipe e in ListaEquipes)
            {
                file.WriteLine($"{e.IdEquipe};{e.Nome};{e.Imagem}");
